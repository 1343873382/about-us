 ...
 * @LastEditTime: 2021-03-09 18:49:40
...
const sreList = [
  ...
  { pos: 43 + offsetMainTitle, opacity: 2, txt: "k8s运维方向:", scle: true, fontSize: '30px' },
  { pos: 47 + offsetTitle, opacity: 2, txt: "招募要求:", scle: true, fontSize: '20px' },
  { pos: 53 + offset, opacity: 2, txt: "1.熟知Docker相关指令，能自行构建Docker镜像、编写docker-compose。", scle: true, fontSize: '15px' },
  { pos: 57 + offset, opacity: 2, txt: "2.了解K8S集群基本概念和原理，熟悉K8S的基本组件 包含但不限于deployment、ingress、helm等。", scle: true, fontSize: '15px' },
  {
    pos: 61 + offset, opacity: 2, txt: "3.了解ceph的基本操作。", scle: true, fontSize: '15px'
  },
  { pos: 65 + offset, opacity: 2, txt: "4.能分析日志、追查并解决问题。", scle: true, fontSize: '15px' },
  { pos: 74 + offsetMainTitle, opacity: 6, txt: "DBA方向:", scle: true, fontSize: '30px' },
  { pos: 78 + offsetTitle, opacity: 6, txt: "招募要求:", scle: true, fontSize: '20px' },
  { pos: 84 + offset, opacity: 6, txt: "1.熟悉基础的sql语句。", scle: true, fontSize: '15px' },
  { pos: 88 + offset, opacity: 6, txt: "2.知道MySQL复制，实时备份、负载均衡等技术", scle: true, fontSize: '15px' },
  { pos: 92 + offset, opacity: 6, txt: "3.了解核心参数以及调整。", scle: true, fontSize: '15px' },
  { pos: 100 + offsetMainTitle, opacity: 8, txt: "网络运维方向:", scle: true, fontSize: '30px' },
  { pos: 104 + offsetTitle, opacity: 8, txt: "招募要求:", scle: true, fontSize: '20px' },
  { pos: 110 + offset, opacity: 8, txt: "1.有基本的网络知识，熟悉 OSI 模型。", scle: true, fontSize: '15px' },
  { pos: 114 + offset, opacity: 8, txt: "2.熟悉TCP/IP协议、NAT、VPN等协议及相关技术。", scle: true, fontSize: '15px' },
  { pos: 118 + offset, opacity: 8, txt: "3.熟悉 Linux 网络配置以及 iptables 相关操作。", scle: true, fontSize: '15px' },
  { pos: 122 + offset, opacity: 8, txt: "4.了解常见品牌路由、交换机的配置。", scle: true, fontSize: '15px' },
  { pos: 126 + offset, opacity: 8, txt: "5.自己折腾过家里网络环境的优先（如全屋万兆改造）。", scle: true, fontSize: '15px' },
  { pos: 132 + offsetMainTitle, opacity: 12, txt: "Devops方向:", scle: true, fontSize: '30px' },
  { pos: 136 + offsetTitle, opacity: 12, txt: "招募要求:", scle: true, fontSize: '20px' },
  { pos: 142 + offset, opacity: 12, txt: "1.知道gitlab 以及gitlab ci。", scle: true, fontSize: '15px' },
  { pos: 146 + offset, opacity: 12, txt: "2.掌握Docker/k8s/pve的基础操作，搭建以及维护。", scle: true, fontSize: '15px' },
  { pos: 150 + offset, opacity: 12, txt: "3.实践过CI/CD持续集成和自动化经验者优先。", scle: true, fontSize: '15px' },
  { pos: 154 + offset, opacity: 12, txt: "4.能够运用主机监控、日志分析 追查问题 提高稳定性。", scle: true, fontSize: '15px' },
  { pos: 176, opacity: 14, txt: "招募对象:", scle: true, fontSize: '30px' },
  { pos: 180, opacity: 14, txt: "大一或大二在校学生。", scle: true, fontSize: '15px' },
  { pos: 186, opacity: 14, txt: "加入我们:", scle: true, fontSize: '30px' },
  { pos: 190, opacity: 14, txt: "发送你的简历（邮件主题命名：方向-学号-姓名，如：DBA-2020xxxxxx-李田所，在正文中留下你的QQ号码，若有github项目地址则一并写入正文，并以PDF格式发送）至sre@redrock.team如无上述技能，但想加入我们，也欢迎前来报名，我们会酌情考量并进行相关培训（包教会。", scle: true, fontSize: '15px' },
]
const designList = [
  { pos: 2, opacity: 1, txt: "视觉设计部", scle: false, reflect: true, fontSize: '40px' },
  { pos: 20, opacity: 1.5, txt: "招募要求:", scle: true, fontSize: '30px' },
  { pos: 30, opacity: 1.5, txt: "1、有一定美术功底和视觉审美能力，能独立完成作品。", scle: true, fontSize: '15px' },
  { pos: 35, opacity: 1.5, txt: "2、对视觉设计感兴趣，熟悉ps基本操作，有较强的学习能力。", scle: true, fontSize: '15px' },
  { pos: 40, opacity: 1.5, txt: "3、个性乐观积极，善于沟通，有责任心，抗压能力强。", scle: true, fontSize: '15px' },
  { pos: 55, opacity: 1.5, txt: "招募对象:", scle: true, fontSize: '15px' },
  { pos: 60, opacity: 1.5, txt: "大一在校学生。", scle: true, fontSize: '15px' },
  { pos: 75, opacity: 1.5, txt: "加入我们:", scle: true, fontSize: '15px' },
  { pos: 80, opacity: 1.5, txt: "将你的作品集以PDF格式发送至design@redrock.team（邮件主题命名格式为： 姓名-学号-专业-电话，如：于某-2020111111-数字媒体艺术-13345556777）。", scle: true, fontSize: '15px' },
]
const flower = [
  {opacity:1,rotate:-40},
  {opacity:1,rotate:-30},
